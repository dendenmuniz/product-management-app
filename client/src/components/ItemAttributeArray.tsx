...
export const ItemAttributeArray = ({ attribute, values, name, isEditing, onChange }: Props) => {
  ...
  <div className="flex flex-col gap-2 w-full">
    {isEditing ? (
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {values.map((val, i) => (
                <div key={i} className="badge badge-outline gap-1">
                  {val}
                  <button type="button" className="ml-1" onClick={() => handleRemove(i)}>✕</button>
                </div>
              ))}
            </div>
          </fieldset>
      ...
      <div className="flex flex-wrap justify-start gap-2">
        {values.map((val, i) => (
          <span key={i} className="badge badge-ghost text-sm">{val}</span>
        ))}
      </div>
      </fieldset>
    ) : (
