...
router.post('/', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (role !== 'seller') {
      return next(createError('Unauthorized - Only sellers can create products', 403));
    }
...
router.put('/:id', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (product.userId !== userId && role !== 'admin') {
      return next(createError('Unauthorized - You can only update your own products', 403));
    }
...
router.delete('/:id', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (product.userId !== userId && role !== 'admin') {
      return next(createError('Unauthorized - You can only delete your own products', 403));
    }
